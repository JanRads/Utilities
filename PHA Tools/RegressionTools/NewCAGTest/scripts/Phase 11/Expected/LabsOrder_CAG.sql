...
,[OrderDate]
 from 
		(select [LabRequestRoot]
				...
				--,[TimeStamp]
				--,[NewFlag]
				, row_number() over (PARTITION by [LabRequestExtension] order by [LabRequestExtension],[Encounter_Id_Root] desc,[EncounterId_Extension] desc 
				--order by [LabRequestExtension],CollectionDate 
