...
EXEC sp_addlinkedserver
   ...
   -- @datasrc='dbm8-data111\inst02',   /* @datasrc='$(OlapServer)'   analysis server name (machine name) */
...
cast("[Util Dims].[Dims_Hier].[Description].[MEMBER_CAPTION]" as varchar(20)) as  MeasureLevel2Desc,
"[Measures].[Result]"  as Result 


--select * 
from openquery(FDX_3, '  SELECT   { [Measures].[Result] } ON COLUMNS,   {   filter ([Util Dims].[Dims_Hier].[Description].ALLMEMBERS , [Util Dims].[Dim Name]  is [Util Dims].[Dim Name].&[chronic disease populations]   )   }  ON ROWS FROM [Utilization]   where [Util Measures].[Mes Name].&[Admissions] ')
