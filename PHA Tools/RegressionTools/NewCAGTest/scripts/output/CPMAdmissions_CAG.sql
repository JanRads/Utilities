SELECT 1                           AS Bookmark, 
       ...
       Year(enddate)               Discharge_Year, 
       Month(enddate)              Discharge_Month, 
       Day(enddate)                Discharge_Day, 
       Datepart(hour, enddate)     Discharge_Hour, 
       Datepart(minute, enddate)   Discharge_Minute
FROM   dbmclinicalanalyticsgateway.fact.encounters 
WHERE  encounterclassindicator LIKE 'IMP' 
