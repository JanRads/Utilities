...
--select * 
from openquery(FDX_3, '  SELECT  { [Measures].[Result] } ON COLUMNS, {    filter ([Util Dims].[Dims_Hier].[Description].ALLMEMBERS , [Util Dims].[Dim Name]  is [Util Dims].[Dim Name].&[AGE Groups]   )   }  ON ROWS FROM [Utilization]   where [Util Measures].[Mes Name].&[readmissions] ')
   order by 1
