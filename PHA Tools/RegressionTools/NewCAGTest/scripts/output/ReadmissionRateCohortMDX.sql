...
select cast("[Cohorts].[Cohort].[Cohort].[MEMBER_CAPTION]" as varchar(50)) as Cohort,
...
from openquery(FDX_3, 'with member cohortname as [Cohorts].[Cohort].properties("name") select {[Measures].[Count Index Adm],[Measures].[Count Readmissions],[Measures].[Readmissions Rate]} on 0, non empty order([Cohorts].[Cohort].[All].children,cohortname) on 1 from [Readmissions]')
EXEC sp_dropserver 'FDX_3', 'droplogins'