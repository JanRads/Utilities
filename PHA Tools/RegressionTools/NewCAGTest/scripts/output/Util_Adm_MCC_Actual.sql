...
cast("[Util Dims].[Dims_Hier].[Description].[MEMBER_CAPTION]" as varchar(20)) as  MCC,
"[Measures].[Result]"  as Result 


--select * 
from openquery(FDX_3, '  SELECT NON EMPTY { [Measures].[Result] } ON COLUMNS, NON EMPTY {    filter ([Util Dims].[Dims_Hier].[Description].ALLMEMBERS , [Util Dims].[Dim Name]  is [Util Dims].[Dim Name].&[mcc score categories]   )   }  ON ROWS FROM [Utilization]   where [Util Measures].[Mes Name].&[Admissions] ')
