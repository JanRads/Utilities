...
from openquery(FDX_3, '  WITH     MEMBER [Measures].[score_1] AS [Measures].[score] *10000000000  SELECT NON EMPTY {  [Measures].[Numerator],[Measures].[Denominator] , [Measures].[Score_1] } ON COLUMNS  ,  { ([Days to readmission].[DTR Group].[DTR Group].ALLMEMBERS ) }  ON ROWS  FROM ( SELECT ( { [Generic Measures].[CalcName].&[15] } ) ON COLUMNS FROM [Readmissions])  WHERE ( [Generic Measures].[CalcName].&[15] )  ')
