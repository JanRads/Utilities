...
cast("[Measures].[Numerator]" as int) as readmission_rate_out_numerator,
cast("[Measures].[Denominator]" as int) as readmission_rate_out_denumerator,
cast("[Measures].[score]" as decimal (5, 2)  ) as readmission_rate_out_score
--select * 
from openquery(FDX_3, '  SELECT NON EMPTY { [Measures].[Numerator], [Measures].[Score], [Measures].[Denominator] } ON COLUMNS  , 
NON EMPTY { filter(([Date Discharge Index Adm].[Month Year].[Month Year].ALLMEMBERS ) ,( [Measures].[Numerator]  +[Measures].[Denominator])>0) }
 ON ROWS FROM ( SELECT ( { [Generic Measures].[CalcName].&[15] } ) ON COLUMNS FROM [Readmissions]) WHERE ( [Generic Measures].[CalcName].&[15] ) ')
 )T
 where T.Year_Month not like 'cur_mo'
