...
cast("[Util Dims].[Dims_Hier].[Description].[MEMBER_CAPTION]" as varchar(20)) as Indicators,
 "[Measures].[Result]"  as Result 


--select * 
from openquery(FDX_3, '  SELECT  { [Measures].[Result] } ON COLUMNS,  {    filter ([Util Dims].[Dims_Hier].[Description].ALLMEMBERS , [Util Dims].[Dim Name]  is [Util Dims].[Dim Name].&[Condition Groups]   )   }  ON ROWS FROM [Utilization]   where [Util Measures].[Mes Name].&[Admissions] ')
   order by 1
