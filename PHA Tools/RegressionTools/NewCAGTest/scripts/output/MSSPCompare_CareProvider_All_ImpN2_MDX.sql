...
select 
cast("[MSSP  Dim].[MSSP Key].[MSSP Key].[MEMBER_CAPTION]" as varchar(40)) as MeasureName,
...
--,cast(" [Measures].[cp_filt]" as int ) as [cp_filt]
...
set [dim3_leafset] as {[Care Providers].[Care Provider].[Care Provider].AllMembers}
set [dim4_leafset] as {[Junc Dim MAN].[Care Provider Key].[Care Provider Key].AllMembers}
set [dim5_leafset] as {[Junc Dim MAN].[Organization Key].[Organization Key].AllMembers}
set [starting_set] as Filter(CROSSJOIN(CROSSJOIN(CROSSJOIN(CROSSJOIN(CROSSJOIN({[MSSP  Dim].[MSSP Key].&[5]}, [dim1_leafset]), [dim2_leafset]), [dim3_leafset]), [dim4_leafset]), [dim5_leafset]),
 [Measures].[Measure Population Ind] <> 0.0)

set [final_set] as {Generate([starting_set], CROSSJOIN(CROSSJOIN(CROSSJOIN(CROSSJOIN(CROSSJOIN({[MSSP  Dim].[MSSP Key].CurrentMember},
 {[MSSP  Dim].[Group Name].CurrentMember}), {[Junc Dim].[comb mssp].CurrentMember}), {[Care Providers].[Care Provider].CurrentMember}),
  {[Junc Dim MAN].[Care Provider Key].CurrentMember}),
...
select {[Measures].[MeasureScore], [Measures].[PrevMeasureScore], [Measures].[Measure Member Ind],
 [Measures].[Measure Population Ind], [Measures].[ImpN],
  [Measures].[AttributedPatientsScore], [Measures].[mssp_precentile],
   [Measures].[mssp_points], [Measures].[mssp_target], [Measures].[gap],
    [Measures].[PrevDenominator], [Measures].[cp_filt]}
	 on columns,
non empty hierarchize({[final_set]}) on rows
from [MSSP]
where ([Date].[Month Year].&[2015_02], [Junc Dim MAN].[Cpo].&[A_A])
