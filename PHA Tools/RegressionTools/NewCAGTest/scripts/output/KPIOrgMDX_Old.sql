...
declare @Myquery varchar (max) = 'select cast("[Measures].[MeasureKey]" as nvarchar(500)) as MeasureKey
,cast("[Measures].[OrgKey]" as int) as [org_RelatedProvider]
, cast(Round("[Measures].[KPI Score]",3) as money) as KPIScore from openquery(FDX_3,'' 
...
select{OrgKey,MeasureKey,[Measures].[KPI Score]} on 0, 
NONEMPTY(order(except([Monitored KPI].[Measure Key].[all].children,[Monitored KPI].[Measure Key].[all].unknownmember),MeasureKey)
* except([Organization].[Organization Key].[all].children,[Organization].[Organization Key].&[-2])) on 1 from [CPM]  
