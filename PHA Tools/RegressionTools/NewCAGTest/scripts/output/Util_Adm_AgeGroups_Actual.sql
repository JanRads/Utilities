...
--select * 
from openquery(FDX_3, '  SELECT NON EMPTY { [Measures].[Result] } ON COLUMNS, NON EMPTY {    filter ([Util Dims].[Dims_Hier].[Description].ALLMEMBERS , [Util Dims].[Dim Name]  is [Util Dims].[Dim Name].&[Age Groups]   )   }  ON ROWS FROM [Utilization]   where [Util Measures].[Mes Name].&[Admissions] ')
